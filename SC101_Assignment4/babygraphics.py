...
def get_x_coordinate(width, year_index):
    ...
    """
    x_coordinate = int(((width-GRAPH_MARGIN_SIZE*2) / len(YEARS)) * int(year_index))+GRAPH_MARGIN_SIZE
    return x_coordinate
    ...
    for i in range(len(lookup_names)):
        ...
        for j in range(len(YEARS)):
            ...
            if str(YEARS[j]) in name_data[lookup_names[i]]:
                y = GRAPH_MARGIN_SIZE+ (int(name_data[lookup_names[i]][str(YEARS[j])])-1)*\
                    (CANVAS_HEIGHT-2*GRAPH_MARGIN_SIZE) / MAX_RANK
                text = str(name_data[lookup_names[i]][str(YEARS[j])])
            else:
                y = CANVAS_HEIGHT-GRAPH_MARGIN_SIZE
                text = '*'
            xy_lst.append(x)
            xy_lst.append(y)
            rank_lst.append(text)
            color_index = i % len(COLORS)
            canvas.create_text(x+TEXT_DX, y, text=lookup_names[i]+' '+text, anchor=tkinter.SW,
                               fill=COLORS[color_index])
        canvas.create_line(xy_lst, fill=COLORS[color_index], width=LINE_WIDTH)
